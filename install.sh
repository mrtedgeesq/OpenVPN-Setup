...
./build-ca < /home/`loguser`/OpenVPN-Setup/ca_info.txt
...
sed 's/LOCALIP/'$LOCALIP'/' </home/`loguser`/OpenVPN-Setup/server_config.txt >/etc/openvpn/server.conf
...
sed 's/LOCALIP/'$LOCALIP'/' </home/`loguser`/OpenVPN-Setup/firewall-openvpn-rules.txt >/etc/firewall-openvpn-rules.sh
...
sed 's/PUBLICIP/'$PUBLICIP'/' </home/`loguser`/OpenVPN-Setup/Default.txt >/etc/openvpn/easy-rsa/keys/Default.txt

# Make directory under home directory for .ovpn profiles
mkdir /home/`loguser`/ovpns
chmod 777 -R /home/`loguser`/ovpns

# Make other scripts in the package executable
cd /home/`loguser`/OpenVPN-Setup
